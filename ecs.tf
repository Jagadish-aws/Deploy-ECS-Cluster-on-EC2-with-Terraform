...
resource "aws_ecs_capacity_provider" "test" {
  ...
  auto_scaling_group_provider {
    ...
    managed_scaling {
      status          = "ENABLED"
      target_capacity = 85   # Sets the target capacity utilization percentage to 85%. This means ECS will try to keep the ASG at 85% capacity by scaling the number of instances up or down as needed.
    }
  }
}
